/**
 * /hires/new — open a new request. Lightweight form; the detail page
 * is where you fill in the rest of the fields once it's been created.
 */
...
export default async function NewHirePage() {
  ...
  async function action(formData: FormData) {
    ...
    if (r.ok && r.id) redirect(`/hires/${r.id}`);
  }
    ...
    >
      <form action={action} className="space-y-5 max-w-2xl">
        <Field
          ...
          name="candidateName"
          required
          placeholder="Derek Smith"
        />
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Email (optional)" name="candidateEmail" type="email" placeholder="derek@example.com" />
        ...
        <Field label="Role title (optional)" name="roleTitle" placeholder="Trail Guide / Horse Handler" />
        <Field label="For which park or unit?" name="forUnit" placeholder="Black Hills RV Park" />
        ...
        <div>
          ...
          <textarea
            name="rolesAndDuties"
            rows={6}
            placeholder="What will this person actually do day-to-day? Schedule? Comp range? Anything Marco/Kevin/Reza should know upfront."
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <Link href="/hires" className="text-sm text-muted hover:text-foreground">Cancel</Link>
        </div>
      </form>
    </PageShell>
  );
