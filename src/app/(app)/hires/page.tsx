/**
 * /hires — leadership-only New Hire Request workflow.
 *
 * Per Reza's process:
 *   1. Marco / a park manager opens a request with role + duties.
 *   2. Kevin reviews finance / tax / legal.
 *   3. Reza reviews company effects / clauses / risks.
 *   4. Back to requester for final remarks.
 *   5. Finalize → contract goes to the candidate (off-CRM).
 *
 * Gated by view_hires permission — defaults: admin, Sales & Marketing,
 * Operations, Finance, Park Manager.
 */
...
export default async function HiresPage({
    ...
    .where(
      and(
        ...
        filter === "all"
          ? undefined
          : filter === "active"
            ? undefined // handled in SQL fragment below
            : eq(hireRequests.status, filter),
      ),
    )
    .orderBy(asc(hireRequests.finalizedAt), desc(hireRequests.createdAt));

  // Filter "active" client-side (small list) rather than building a more
  // complex SQL WHERE — keeps the query readable.
  const visibleRows = filter === "active"
    ? rows.filter((r) => (ACTIVE_STATUSES as readonly string[]).includes(r.status))
    : rows;

  // Counts across all (non-deleted) rows for the chip row.
  const all = await db
  ...
  return (
    <PageShell
      title="New Hires"
      subtitle="Vet a candidate through the leadership workflow before the contract goes out."
      action={
        canManage ? (
          <Link
            href="/hires/new"
    ...
    >
      ...
      <div className="flex items-center gap-1.5 mb-5 flex-wrap">
        <Chip href="/hires" active={filter === "active"}>
          Active <span className="ml-1 text-foreground/50 tabular-nums">· {activeCount}</span>
        </Chip>
        <Chip href="/hires?s=all" active={filter === "all"}>
        ...
        {(["draft", "finance_review", "founder_review", "requester_review", "finalized", "withdrawn"] as const).map((s) => (
          <Chip key={s} href={`/hires?s=${s}`} active={filter === s}>
              ...
              {visibleRows.map((r) => (
                <tr key={r.id} className="border-t border-border hover:bg-foreground/[0.02] transition">
                  <td className="px-3 py-2.5 font-medium">
                    <Link href={`/hires/${r.id}`} className="hover:underline">
                      {r.candidateName}
                    </Link>
                  </td>
