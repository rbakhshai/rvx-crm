...
export default function ContactDrawerLoading() {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-stretch justify-end animate-in fade-in duration-150">
      ...
      <div className="relative h-screen bg-background border-l border-border shadow-2xl overflow-y-auto animate-in slide-in-from-right duration-200" style={{ width: "min(600px, 92vw)" }}>
        <header className="sticky top-0 z-10 flex items-center gap-3 border-b border-border bg-background/95 backdrop-blur px-5 py-3">
          <div className="flex-1 space-y-1"><Skeleton className="h-3 w-40" /><Skeleton className="h-2.5 w-24" /></div>
        </header>
        <div className="px-5 py-4 space-y-4">
          <div className="flex gap-2"><Skeleton className="h-6 w-12" /><Skeleton className="h-6 w-16" /></div>
          <div className="grid grid-cols-2 gap-4">{Array.from({ length: 6 }).map((_, i) => <div key={i}><Skeleton className="h-2 w-16" /><Skeleton className="h-4 w-24 mt-1" /></div>)}</div>
        </div>
      </div>
    </div>
  );
